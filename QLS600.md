...
The goal is to identify the potential target genes of a given transcription facor.

You need to:
1. Select a transcription factor ChIP-seq dataset in a particular tissue and retrieve its peaks from https://www.encodeproject.org/. You don't necessarily need to call peaks, you could use the peak files directly.
2. Identify genes that are highly expressed in the same tissue either from https://www.encodeproject.org/ or from https://www.gtexportal.org/
3. Use bedtools or a similar tool to identify peaks that are within 10Kb of an expressed gene in that tissue
4. Prepare a 2-3 page summary report of what you found, including an example target gene genome browser figure (with RNA-seq and ChIP-seq tracks)

Make sure all your datasets use the same genome assembly (e.g. hg19).
